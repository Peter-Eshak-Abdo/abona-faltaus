...
export default function ChatBot() {
  ...
  // 1. الدستركتشرنج الصحيح لـ useChat (لا داعي لـ Transport أو chat.x)
  const {
    messages,
    setMessages,
    input,
  ...
  const handleSelect = async (id: string) => {
    ...
    if (!error && msgs) {
      setMessages(
        msgs.map((m: { id: any; role: string; content: any; }) => ({
          id: m.id,
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      setInput(""); // تفريغ الـ Input لسرعة الاستجابة
      ...
      // 3. نداء مباشر لـ append بدون مشاكل
  ...
  // عمل Scroll تلقائي مع نزول الرسائل
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50" dir="rtl">
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="right" className="w-72 flex flex-col p-0">
          <SheetHeader className="p-4 border-b">
            ...
            <Button onClick={handleNew} variant="outline" className="w-full mt-2 gap-2 border-amber-200">
          ...
          <ScrollArea className="flex-1">
            {convs.map((c) => (
              <div
                key={c.id}
                className={cn(
                  "p-3 cursor-pointer hover:bg-amber-50 flex justify-between items-center group border-b",
                  convId === c.id && "bg-amber-100/50"
                )}
                onClick={() => handleSelect(c.id)}
              >
                <span className="text-sm truncate flex-1 ml-2 text-right">{c.title}</span>
                <Trash2
                  size={14}
      ...
      <header className="flex items-center justify-between p-3 bg-amber-600 text-white shadow-md z-10">
            ...
            {messages.map((m: { role: string; content: string | Node; }, idx: number) => (
              <div key={idx} className={cn("flex", m.role === "user" ? "justify-start" : "justify-end")}>
                <div className={cn(
                  ...
                  m.role === "user" ? "bg-amber-500 text-white rounded-tr-none" : "bg-white border text-gray-800 rounded-tl-none"
                )}>
                  {/* {m.role === "user" ? (
                    m.content
                  ) : ( */}
      ...
      <footer className="p-4 border-t bg-white">
        <form onSubmit={handleSend} className="max-w-2xl mx-auto flex gap-2">
          <Textarea
            ...
            placeholder={user ? "اكتب سؤالك الروحي..." : "برجاء تسجيل الدخول..."}
            className="flex-1 min-h-[44px] max-h-32 resize-none border-amber-100"
            disabled={!user || isLoading}
            onKeyDown={(e) => {
              if (e.key === "Enter" && !e.shiftKey) {
                e.preventDefault();
                handleSend(e); // نمرر الـ event
              }
            }}
          />
          <Button
            ...
            className="bg-amber-600 hover:bg-amber-700 h-11 w-11 p-0 rounded-xl"
          >
            <Send size={18} />
          </Button>
        </form>
        {!user && <p className="text-center text-[10px] text-red-500 mt-2 font-bold">تنبيه: يجب تسجيل الدخول ليعمل الشات</p>}
      </footer>
    </div>
  );
}
