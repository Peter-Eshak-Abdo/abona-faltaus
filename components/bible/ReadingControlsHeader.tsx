...
export default function ReadingControlsHeader({
  ...
  return (
    <header className="sticky bg-surface-container-low/95 backdrop-blur-xl z-30 px-1 flex items-center justify-between border-b border-surface-variant/30 flex-col md:flex-row shadow-2xl rounded-4xl pb-0.5">
      ...
      <div className="flex flex-wrap gap-0.5 mt-0.5 px-0.5 z-20 justify-between">
        <button onClick={() => setFontSize(prev => prev + 2)} className="p-0.5 bg-surface-container text-on-surface rounded-lg font-bold">A+</button>
        <button onClick={() => setFontSize(prev => prev - 2)} className="p-0.5 bg-surface-container text-on-surface rounded-lg font-bold">A-</button>

        <Link href="/bible/favorites" className="p-0.5 bg-yellow-100 text-yellow-600 rounded-lg font-bold flex items-center justify-center" title="المفضلة">
          <FaStar size={18} />
        </Link>

        <Link href="/bible/day" className="p-0.5 bg-amber-200 text-blue-600 rounded-lg font-bold flex items-center justify-center" title="إضافة إلى يوم">
          <FaPlusSquare size={18} />
        </Link>

        <button onClick={() => setIsSearchOpen(true)} className="p-0.5 bg-blue-100 text-blue-600 rounded-lg font-bold flex items-center justify-center">
          <FaSearch size={18} />
        </button>
        ...
        <button
          ...
          className={`flex-1 sm:flex-none flex items-center justify-center gap-0.5 p-0.5 rounded-full font-bold transition-all disabled:opacity-50
