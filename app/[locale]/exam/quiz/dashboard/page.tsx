        ...
        if (validIds.length > 0) {
          try {
            const { data: byId } = await supabase
              ...
              .select("*")
              .in("id", validIds);
            if (byId) historyQuizzes.push(...byId);
          } catch (e) {
            console.warn("Fetch byId error:", e);
          }
        }
        ...
        if (validCodes.length > 0) {
          try {
            const { data: byCode } = await supabase
              ...
              .select("*")
              .in("code", validCodes);
            if (byCode) historyQuizzes.push(...byCode);
          } catch {
            // fallback if code column is not directly filterable with in()
          }

          try {
            const { data: byAdminCode } = await supabase
              .from("quizzes")
              .select("*")
              .in("admin_code", validCodes);
            if (byAdminCode) historyQuizzes.push(...byAdminCode);
          } catch {
            // ignore
          }
        }
