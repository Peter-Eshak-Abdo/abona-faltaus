...
const MeladPlayer: React.FC = () => {
  return (
    <div id="sec-melad" className="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
      {sounds
        .filter((sound) => sound.monasba === "melad")
        .map((sound, index) => (
          <div className="col" key={`melad-${index}`}>
            <div className="card shadow-sm pt-3 ps-3" key={`melad-${index}`}>
              <div className="d-flex justify-content-center align-items-center">
                <audio controls is="x-audio" id={`melad-audio-${index}`}>
              ...
              <div className="card-body text-start">
                ...
                <p className="card-text">
                  <span className="fw-bold">المناسبة التي يقال فيها: </span>عيد الميلاد
                </p>
